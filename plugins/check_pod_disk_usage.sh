...
oc get pod -o json | jq -r '.items[] | select(.status.phase == "Running") | .metadata | [.namespace, .name] | @tsv' | sort | while IFS=; read podns; do
	ns=`echo $podns | awk '{print $1}'`
	pod=`echo $podns | awk '{print $2}'`
	oc exec $pod -n $ns -- bash -c "$DF_CMD | grep -v Filesystem" | while read line; do
	  echo -e "$ns\t$pod\t$line" >> $TEMP_FILE 
	done
done
